...
namespace GlobalGameJam
{
    ...
    public class MinimapGridViewModel : MonoBehaviour
    {
        ...
        [Header("Data Safety")]
        [SerializeField] private bool autoScanIfNeeded = true;
        ...
        {
            Vector2Int centerGridPos = playerGridPos;
            if (!centerOnPlayer)
            {
                centerGridPos = new Vector2Int(gridModel.GridWidth / 2, gridModel.GridHeight / 2);
            }

            int gridWidth = gridModel.GridWidth;
            int gridHeight = gridModel.GridHeight;

            int desiredStartX = centerGridPos.x - viewportWidth / 2;
            int desiredStartY = centerGridPos.y - viewportHeight / 2;
        ...
        {
            if (gridScanner != null && gridScanner.GridModel != null && gridScanner.GridModel != gridModel)
            {
                gridModel = gridScanner.GridModel;
            }

            if (gridModel == null)
                return;

            bool hasData = gridModel.IsInitialized && gridModel.HasAnyNonEmptyCells();
            if (!hasData)
            {
                if (autoScanIfNeeded && gridScanner != null)
                {
                    gridScanner.ScanScene();
                }

                gridModel.EnsureInitialized();
            }
        }
